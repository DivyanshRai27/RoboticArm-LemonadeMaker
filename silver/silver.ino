...
void setup() {
  ...
  motor1.setSpeed(75);//lift right --> FORWARD is arm going down
  motor2.setSpeed(150);//rotate --> BACKWARD is clk wise movement
  motor3.setSpeed(200);//left lift --> FORWARD is arm going down
  motor4.setSpeed(75);//hand  --> FORWARD is closing of hand
  motor1.run(RELEASE);
  motor2.run(RELEASE);
  motor3.run(RELEASE);
  motor4.run(RELEASE);
...
void loop() {
  ...
  motor1.run(BACKWARD);
  motor3.run(BACKWARD);
  delay(1000);                  /*change this delay to modify time for how long arm should go up*/
 ...
 motor4.run(BACKWARD); //hand is set to close. so in reference state hand will be closed
  ...
  motor4.run(FORWARD); //arm hand set to open
